...
function ChangeoverRelay({ label, active, psLabel, wire }) {
  return (
...
function CutoffRelay({ label, gpioPin, psLabel, isCut }) {
  return (
    <div className={`flex flex-col items-center gap-2 px-4 py-3 rounded-lg border transition-colors duration-500 ${
...
export default function RelayPanel({ relays, cutoff }) {
  return (
    <div className="rounded-xl border border-zinc-800 bg-zinc-900 p-5 flex flex-col gap-5">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        <ChangeoverRelay label="R1" psLabel="PS1" wire="L-wire" active={relays.ps1_l} />
        <ChangeoverRelay label="R2" psLabel="PS1" wire="N-wire" active={relays.ps1_n} />
        <ChangeoverRelay label="R3" psLabel="PS2" wire="L-wire" active={relays.ps2_l} />
        <ChangeoverRelay label="R4" psLabel="PS2" wire="N-wire" active={relays.ps2_n} />
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <CutoffRelay label="R5" gpioPin="24" psLabel="PWR1" isCut={cutoff?.ps1 || relays.ps1_cut} />
        <CutoffRelay label="R6" gpioPin="25" psLabel="PWR2" isCut={cutoff?.ps2 || relays.ps2_cut} />
      </div>
